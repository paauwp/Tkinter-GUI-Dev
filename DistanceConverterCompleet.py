...
class RootClass(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        containerObject = ttk.Frame(self)
        containerObject.grid(padx=10, pady=10, sticky="EW")

        for FrameClass in (MetresToFeetClass, FeetToMetres):
            frameObject = FrameClass(containerObject, self)
            self.frames[FrameClass] = frameObject
            frameObject.grid(row=0, column=0, sticky="NSEW")

        self.show_frame(MetresToFeetClass)
...
class MetresToFeetClass(ttk.Frame):
...
class FeetToMetres(ttk.Frame):
    def __init__(self, container, controller):
        ...
        switch_page_button = ttk.Button(
            ...
            command=lambda: controller.show_frame(MetresToFeetClass)
        )
        switch_page_button.grid(column=1, row=4, columnspan=2, sticky="EW")
